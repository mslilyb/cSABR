...
def random_spots(iemin, iemax, iestep):
	sizes = [num for num in range(iemax, iemin - 1, -iestep)]
...
for c in range(arg.chroms):
	...
	off = 0
	shuff_esizes = random_spots(arg.emin, arg.emax, arg.estep)
	for shuff_esize in shuff_esizes:
		for strand in strands:
			for site in sites:
				...
				ve = random_exon(shuff_esize, arg.debug)
				...
				off += len(seq)
				print(seq, file=fa)
				ftx = FTX(chrom, f'{shuff_esize}:{site}', strand, exons, '')
				print(ftx, file=fx)
